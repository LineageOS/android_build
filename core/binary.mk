...
ifneq ($(filter true always, $(LOCAL_FDO_SUPPORT)),)
  ...
  ifeq ($(is_cc_ccache),true)
    ...
    cc_ccache_ge_3_2 = $(shell if [ $(vmajor) -gt 3 -o $(vmajor) -eq 3 -a $(vminor) -ge 2 ]; then echo true; else echo false; fi)
  endif
  is_cxx_ccache := $(shell if [ "`$(my_cxx_wrapper) -V 2>/dev/null | head -1 | cut -d' ' -f1`" = ccache ]; then echo true; fi)
  cxx_ccache_ge_3_2 := false
  ifeq ($(is_cxx_ccache),true)
    ...
    cxx_ccache_ge_3_2 = $(shell if [ $(vmajor) -gt 3 -o $(vmajor) -eq 3 -a $(vminor) -ge 2 ]; then echo true; else echo false; fi)
  endif

  ifeq ($(cc_ccache_ge_3_2),false)
    my_cc_wrapper := $(filter $(GOMA_CC),$(my_cc_wrapper))
  endif
  ifeq ($(cxx_ccache_ge_3_2),false)
