...
@mkdir -p $(dir $@)
@echo -e ${CL_GRN}"Sed:"${CL_RST}" $(if $(PRIVATE_MODULE),$(PRIVATE_MODULE),$@) <= $<"
...
@mkdir -p $(dir $@)
@echo -e ${CL_GRN}"Lex:"${CL_RST}" $(PRIVATE_MODULE) <= $<"
$(hide) $(LEX) -o$@ $<
endef
...
@mkdir -p $(dir $@)
@echo -e ${CL_GRN}"Yacc:"${CL_RST}" $(PRIVATE_MODULE) <= $<"
...
$(foreach lib,$(PRIVATE_ALL_WHOLE_STATIC_LIBRARIES), \
	@echo -e ${CL_GRN}"preparing StaticLib:"${CL_RST}" $(PRIVATE_MODULE) [including $(lib)]"; \
...
$(foreach lib,$(PRIVATE_ALL_WHOLE_STATIC_LIBRARIES), \
	@echo -e ${CL_YLW}"preparing StaticLib:"${CL_RST}" $(PRIVATE_MODULE) [including $(lib)]"; \
	ldir=$(PRIVATE_INTERMEDIATES_DIR)/WHOLE/$(basename $(notdir $(lib)))_objs;\
	rm -rf $$ldir; \
	mkdir -p $$ldir; \
	filelist=; \
...
@mkdir -p $(dir $@)
@echo -e ${CL_GRN}"target Prelink:"${CL_RST}" $(PRIVATE_MODULE) ($@)"
$(hide) $(APRIORI) \
