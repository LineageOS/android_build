...
function eat()
{
    if [ "$OUT" ] ; then
        ...
        if [ $(adb get-state) != device -o $(adb shell busybox test -e /sbin/recovery;echo $?) != 0 ] ; then
            echo "No device is online. Waiting for one..."
            echo "Please connect USB and/or enable USB debugging"
            until [ $(adb get-state) = device -o $(adb shell busybox test -e /sbin/recovery;echo $?) = 0 ];do
                sleep 1
            done
            echo "Device Found.."
        fi
