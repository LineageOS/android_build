...
PARTITIONS_WITH_CARE_MAP = ('system', 'vendor', 'product', 'odm')
...
def AddOdm(output_zip):
  """Turn the contents of ODM into a odm image and store it in
  output_zip."""
...
def AddImagesToTargetFiles(filename):
  ...
  # {vendor,product,odm}.img is unlike system.img or system_other.img. Because it
  # could be built from source, or dropped into target_files.zip as a prebuilt
  # blob. We consider either of them as {vendor,product,odm}.img being available,
  ...
  has_odm = (os.path.isdir(os.path.join(OPTIONS.input_tmp, "ODM")) or
             os.path.exists(os.path.join(OPTIONS.input_tmp, "IMAGES", "odm.img")))
