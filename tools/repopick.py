...
def fetch_query_via_ssh(remote_url, query):
    ...
    if "@" in remote_url:
        (username, hostname) = remote_url.split("@")
        if ':' in hostname:
            (hostname, port) = hostname.split(":")
        else:
            port = 29418
    else:
        raise Exception("SSH URL should be in the form user@hostname[:port]")

    out = subprocess.check_output(['ssh', '-x', '-p{0}'.format(port), '{0}@{1}'.format(username, hostname), 'gerrit', 'query', '--format=JSON --patch-sets --current-patch-set {0}'.format(query)])
        ...
        if line and line[0] == '{':
            ...
            if 'type' not in data:
                ...
                review = {
                    ...
                    u'change_id': data['id'],
                    u'current_revision': data['currentPatchSet']['revision'],
                    u'number': int(data['number']),
                    u'revisions': {patch_set['revision']: {
                        u'number': int(patch_set['number']),
                        u'fetch': {
                            u'anonymous http': {
                                u'ref': patch_set['ref'],
                                u'url': u'ssh://{0}@{1}:{2}/{3}'.format(username, hostname, port, data['project'])
                            }
                        }
...
def fetch_query_via_http(remote_url, query):
    ...
    print(url)
    reviews = json.loads(urllib.request.urlopen(url).read().decode("utf-8")[5:])
...
if __name__ == '__main__':
    ...
    for item in mergables:
        ...
        if args.pull:
            cmd = ['cd', project_path, '&& git pull --no-edit github', item['fetch']['anonymous http']['ref']]
        else:
            cmd = ['cd', project_path, '&& git fetch github', item['fetch']['anonymous http']['ref']]
        ...
        if os.stat(FETCH_HEAD).st_size == 0:
            ...
            if args.pull:
                cmd = ['cd', project_path, '&& git pull --no-edit', item['fetch']['anonymous http']['url'], item['fetch']['anonymous http']['ref']]
            else:
                cmd = ['cd', project_path, '&& git fetch', item['fetch']['anonymous http']['url'], item['fetch']['anonymous http']['ref']]
            subprocess.call(cmd, shell=True)
        # Perform the cherry-pick

        if not args.pull:
            cmd = ['cd', project_path, '&& git cherry-pick FETCH_HEAD']
            subprocess.call(cmd, shell=True)
        if not args.quiet:
            print('')