...
if __name__ == '__main__':
    ...
    for item in mergables:
        ...
        if args.pull:
            cmd = ['cd', project_path, '&& git pull --no-edit github', item['fetch'][method]['ref']]
        else:
            cmd = ['cd', project_path, '&& git fetch github', item['fetch'][method]['ref']]
        ...
        if os.stat(FETCH_HEAD).st_size == 0:
            ...
            if args.pull:
                cmd = ['cd', project_path, '&& git pull --no-edit', item['fetch'][method]['url'], item['fetch'][method]['ref']]
            else:
                cmd = ['cd', project_path, '&& git fetch', item['fetch'][method]['url'], item['fetch'][method]['ref']]
            subprocess.call(cmd, shell=True)
