...
for change in args.change_number:
    ...
    if project_name in project_name_to_path:
        ...
        if project_path.startswith('hardware/qcom/audio') or project_path.startswith('hardware/qcom/display') or project_path.startswith('hardware/qcom/media'):
            if project_branch.split('-')[0] == 'cm' and project_branch.split('-')[2] == 'caf':
                if "-caf/" in project_branch:
                    project_branch = project_branch.split('-caf')[0] + "/"
                project_path = project_path.rsplit('/', 1)[0] + '-caf/' + 'msm' + project_branch.rsplit('-', 1)[1]
